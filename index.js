  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.url === '/limpiar-sesion' && req.method === 'GET') {
      try {
        await limpiarAuth();
        res.end(`<!DOCTYPE html><html><body style="font-family:sans-serif;text-align:center;padding:60px">
          <h2>Sesi&#243;n borrada</h2>
          <p>Reinici&#225; el servicio en Render → al arrancar te va a pedir el QR de nuevo.</p>
          <p style="color:#888;font-size:.85rem">Despu&#233;s de re-escanear, WhatsApp va a re-sincronizar el historial completo de los &#250;ltimos 15 d&#237;as.</p>
        </body></html>`);
      } catch (err) {
        res.end(`<p>Error: ${err.message}</p>`);
      }
    ...
    if (req.url === '/historial' && req.method === 'GET') {
      ...
      try {
        ...
        res.end(`<!DOCTYPE html><html><body style="font-family:sans-serif;text-align:center;padding:60px">
          ...
          <p style="color:#888;font-size:.85rem">Esto puede tardar unos minutos seg&#250;n el volumen. Vas a recibir las notificaciones en ntfy a medida que se procesen.</p>
          <p style="color:#888;font-size:.8rem">Si no aparecen mensajes hist&#243;ricos, reinici&#225; el servicio en Render para forzar otra sincronizaci&#243;n con WhatsApp.</p>
          <a href="/">Volver</a>
        </body></html>`);
      } catch (err) {
        res.end(`<p>Error: ${err.message}</p>`);
  ...
  try {
    ...
    if (eventos.length || compromisos.length || pedidos.length) {
      ...
      try {
        ...
        console.error(`[Cron individuales] Error enviando a WA:`, err.message);
      }
    }
